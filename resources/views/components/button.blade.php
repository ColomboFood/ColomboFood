...
@if($ghost)
    <button {{ $attributes->merge(['type' => 'submit', 'class' => '
                ...
                focus:border-primary-500 focus:ring focus:ring-primary-400 disabled:opacity-25 transition active:text-white
            ']) }}
        {{ $disabled ? 'disabled' : '' }}        
    >
        {{ $slot }}
...
@else
    <button {{ $attributes->merge(['type' => 'submit', 'class' => '
                ...
                focus:border-primary-500 focus:ring focus:ring-primary-400 disabled:opacity-25 transition
            ']) }}
        {{ $disabled ? 'disabled' : '' }}        
    >
        {{ $slot }}
