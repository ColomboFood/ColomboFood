    ...
    @if ($paginator->hasPages())
        <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
            ...
            <div class="flex-col hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                <div>
                    <p class="mb-2 text-sm leading-5 text-gray-700">
                        @if ($paginator->firstItem())
                            {!! __('pagination.label.interval', [
                    ...
                    <span class="relative z-0 inline-flex rounded-md shadow-sm">
                        ...
                        @else
                            <a href="{{ $paginator->previousPageUrl() }}" rel="prev"
                                class="relative inline-flex items-center px-2 py-2 text-sm font-medium leading-5 text-gray-500 transition duration-150 ease-in-out bg-white border border-gray-300 rounded-l-md hover:text-gray-400 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-primary-300 active:bg-gray-100 active:text-gray-500"
                        ...
                        @foreach ($elements as $element)
                            ...
                            @if (is_array($element))
                                @foreach ($element as $page => $url)
                                    ...
                                    @else
                                        <a href="{{ $url }}"
                                            class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium leading-5 text-gray-700 transition duration-150 ease-in-out bg-white border border-gray-300 hover:text-gray-500 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-primary-300 active:bg-gray-100 active:text-gray-700"
